...
describe("StakeIDrisToken", function () {
    ...
    beforeEach(async function () {
        ...
        const StakeIDrisTokenFactory = await ethers.getContractFactory("StakeIDrisToken");
    ...
    describe("Staking", function () {
        it("Should allow staking and emit event", async function () {
            ...
            const stakerRecord = await stakeIDrisToken.connect(addr1).getStakeRecord();
    ...
    describe("Withdrawing", function () {
        it("Should allow withdrawal with interestAccrued after staking period and emit event", async function () {
            ...
            const interestAccrued = (await stakeIDrisToken.connect(addr1).getStakeRecord())[1];
        ...
        it("Should revert withdrawal if funds are locked", async function () {
            ...
            await expect(stakeIDrisToken.connect(addr1).withdraw()).to.be.revertedWithCustomError(
                stakeIDrisToken,
                "AssetIsLocked",
            );
        });
        ...
        it("Should revert if no record found", async function () {
            await expect(stakeIDrisToken.connect(addr2).withdraw()).to.be.revertedWithCustomError(
                stakeIDrisToken,
                "NoRecordFound",
            );
        });
    ...
    describe("Transfer Stake", function () {
        it("Should allow transfer of stake and emit event", async function () {
            ...
            await expect(stakeIDrisToken.connect(addr1).transferStake(addr2, stakeAmount, 1))
                .to.emit(stakeIDrisToken, "TransferSuccessful")
                .withArgs(addr1, addr2, stakeAmount);

            const stakerRecord = await stakeIDrisToken.connect(addr2).getStakeRecord();
            expect(stakerRecord.staker.amountStaked).to.equal(stakeAmount);
        });

        it("Should revert transfer if transferring to zero address", async function () {
            await expect(
                stakeIDrisToken.connect(addr1).transferStake(ethers.ZeroAddress, stakeAmount, 1),
        ...
        it("Should revert transfer if insufficient funds", async function () {
            ...
            await expect(
                stakeIDrisToken.connect(addr1).transferStake(addr2, stakeAmount + 1n, 1),
            ).to.be.revertedWithCustomError(stakeIDrisToken, "InsufficientFunds");
        });
        ...
        it("Should revert transfer if receiver is already a staker", async function () {
            ...
            ).to.be.revertedWithCustomError(stakeIDrisToken, "CanNotTransferToExistingStaker");
        });
    });
    ...
    describe("Owner Functions", function () {
        ...
        it("Should revert if contract has insufficient balance", async function () {
            ...
            ).to.be.revertedWithCustomError(stakeIDrisToken, "InsufficientContractBalance");
        });
    });
