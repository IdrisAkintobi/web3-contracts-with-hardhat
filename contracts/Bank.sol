...
contract Bank {
    ...
    event ExTransfer(address indexed _from, address indexed _to, uint256 _value);
    ...
    function createAccount(string memory _accName, string memory _accNum) external payable {
        ...
        emit AccountCreated(bankName, msg.sender, _accNum, _accName, uint64(msg.value));
    }

    function deposit() external payable {
        authCheck();
        require(msg.value >= 10_000 wei, "You can not deposit less than 10 000 wei");
    ...
    function authCheck() private view {
        require(bytes(ledger[msg.sender].name).length != 0, "Please register with the bank");
    }

    function createAccCheck(string memory _accName, string memory _accNum) private view {
        require(bytes(_accNum).length >= 10, "Account number must be in NUBAN");
        require(bytes(_accName).length >= 6, "Account name must be 6 characters or more");
        require(
            bytes(ledger[msg.sender].name).length == 0 && customerRecord[_accNum] == address(0),
            "Account already exists"
        );
        require(msg.value >= 100_000 wei, "Opening balance can not be less than 100 000 wei");
    }
}
